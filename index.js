...
if (window && window.document) {
     ...
     * });
        ...
        ScriptAttributes.prototype.set = function(attrs) {
            for(var key in attrs) {
                this.add(key, resolve(attrs[key]))
            }
        }
    ...
    * };
        ...
        * };
            ...
            app = app || pc.Application.getApplication()
            if (app) {
                var registry = app.scripts
                registry.add(script);
            }
    ...
    let oldCreateScript = pc.createScript
    pc.createScript = function(name) {
        let result = oldCreateScript(name)
        result.attributes.set = function(attrs) {
            for(var key in attrs) {
                this.add(key, resolve(attrs[key]))
            }
        }
